...
const countNumberOfElements = arr => arr.reduce(agg => (agg += 1));
...
const extractChildren = arr =>
  arr
    .filter(element => element.name.match(/a/))
    .reduce((agg, val, idx) => {
      if (val.children) {
        val.children.forEach(element => agg.push(element));
      }
      return agg;
    }, []);
