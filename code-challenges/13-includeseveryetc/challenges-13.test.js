...
const firstLetters = arr => arr.map(starter => starter.charAt(0));
...
const standardizePhoneNumbers = arr =>
  arr.map(number => number.replace(/[\(\)\- ]/g, ''));
...
const findAnything = (arr, target) =>
  arr.filter(string => string.includes(target));
...
const unenrollBrook = arr =>
  arr.map(darray => darray.filter(string => !string.includes('Brook')));
...
const sortByDay = arr =>
  daysOfWeek.map(day => arr.filter(activity => activity.includes(day)));
